        ...
        if (ercID == NATIVEID) {
            ...
            if (IERC20(lootToken).balanceOf(address(this)) < amount) {
                revert InsufficientBalance(
                    IERC20(lootToken).balanceOf(address(this)),
                    amount
                );
            }
            ...
            if (
                IERC1155(lootToken).balanceOf(address(this), tokenId) < amount
            ) {
                revert InsufficientBalance(
                    IERC1155(lootToken).balanceOf(address(this), tokenId),
                    amount
                );
            }
            IERC1155(lootToken).safeTransferFrom(
                address(this),
                to,
                tokenId,
                amount,
